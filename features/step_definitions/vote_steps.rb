Given /^(bill "[^\"]*") has the following passage votes(?: on "(.*)"|):$/ do |bill, voted_at, table|
  roll = create(:roll, subject: bill, roll_type: "On Passage", voted_at: voted_at)
  table.map_column!('politician') {|name| Politician.with_name(name).first }
  table.hashes.each do |attrs|
    next if attrs['vote'].blank?
    create(:vote, attrs.symbolize_keys.merge(roll: roll))
  end
end
...
Given /^the following bill passage votes:$/ do |table|
  ...
  table.hashes.each do |attrs|
    ...
    @rolls ||= attrs.keys.inject({}) do |rolls, bill|
      rolls[bill] = create(:roll, subject: Bill.with_title(bill).first, roll_type: "On Passage")
      rolls
    end

    attrs.each_pair do |bill, vote|
      next if vote.blank?
      create(:vote, politician: politician, roll: @rolls[bill], vote: vote)
    end
  end
end
...
Given /^((?:bill|amendment) "[^\"]*") has the following roll votes:$/ do |bill_or_amendment, table|
  ...
  table.hashes.each do |attrs|
    ...
    attrs.each_pair do |roll_type, vote|
      next if vote.blank?
      create(:vote, politician: politician, roll: rolls[roll_type], vote: vote)
    end
  end
end
